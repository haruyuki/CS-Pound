class Dragon:
    ...
    def _get_lineage(self, data):
        parents_temp = data.xpath(
            '//div[@id="super-container"]/div[last()]/div[5]/fieldset/div[2]/div[2]/a'
        )
        ...
        children_temp = data.xpath(
            '//div[@id="super-container"]/div[last()]/div[5]/fieldset/div[2]/div[5]/a'
        )
    ...
    def _get_info(self, data):
        ...
        try:
            breeding_temp = data.xpath(
                '//div[@id="newname"]/fieldset/div[1]/a[@class="loginbar"]/@title'
            )[0]
            self.breeding_available = False
            self.breeding_cooldown = int(
                [n for n in breeding_temp.split() if n.isdigit()][0]
            )
        except IndexError:
            self.breeding_available = True
            self.breeding_cooldown = 0
        ...
        flight_temp = data.xpath(
            '//div[@id="newname"]/fieldset/div[1]/a[@class="elemclue"]/@title'
        )[0]
        self.flight = flight_temp.split()[0]

        gender_temp = data.xpath(
            '//div[@id="newname"]/fieldset/div[1]/a[@class="miniclue"]/@title'
        )[0]
        self.gender = gender_temp.split()[0]

        breed_temp = data.xpath(
            '//div[@id="newname"]/fieldset/div[2]/span[2]/div/div[2]/text()'
        )[0]
        self.breed = breed_temp.split()[0]

    def _get_genes(self, data):
        genes = data.xpath(
            '//div[@id="newname"]/fieldset/div/span[last()]/div/div/text()'
        )
    ...
    class _Stats:
        ...
        def _strength(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[0].xpath("span[2]/text()")[0]
            )
            return value

        def _intelligence(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[1].xpath("span[2]/text()")[0]
            )
            return value

        def _agility(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[2].xpath("span[2]/text()")[0]
            )
            return value

        def _vitality(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[3].xpath("span[2]/text()")[0]
            )
            return value

        def _defense(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[4].xpath("span[2]/text()")[0]
            )
            return value

        def _mind(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[5].xpath("span[2]/text()")[0]
            )
            return value

        def _quickness(self):
            value = int(
                self.data.xpath('//a[@class="cluestat"]')[6].xpath("span[2]/text()")[0]
            )
            return value
    ...
    class _Growth:
        ...
        def __call__(self):
            return {
                ...
                "wingspan": self.raw_wingspan,
                "weight": self.raw_weight,
            }

        def _get_length(self):
            value = self.data.xpath(
                '//div[@id="newname"]/fieldset/div[2]/span[6]/div/span/text()'
            )[0]
