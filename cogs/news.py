...
class News(commands.Cog):
    ...
    async def latest(self, ctx):
        news_articles = (
            await cs.get_announcements()
        )  # Get the HTML list of all the news articles
        ...
        canvas = None
        if (
            latest.find('a/img[@alt="Image"]') is not None
        ):  # If news has click-able images
            if (
                len(latest.findall('a/img[@alt="Image"]')) == 1
            ):  # If there is only 1 image
                image_tag = latest.find('a/img[@alt="Image"]')  # Get the 'img' tag
                image_link = image_tag.xpath("@src")[
                    0
                ]  # Extract image link for use in embed later
                parent = (
                    image_tag.getparent()
                )  # Get parent tag of 'img', which is 'a' tag
                latest.remove(
                    parent
                )  # Remove the 'a' tag so it won't be converted to Markdown
            else:  # If there is more than 1 image
                image_list = latest.findall(
                    'a/img[@alt="Image"]'
                )  # Get the links to all the images
                multiple_images = True
        elif (
            latest.find('img[@alt="Image"]') is not None
        ):  # If the news has static images instead
            if (
                len(latest.findall('img[@alt="Image"]')) == 1
            ):  # If there is only 1 image
                ...
                latest.remove(
                    image_tag
                )  # Remove the 'img' tag so it won't be parsed later
            else:  # If there is more than 1 image
                image_tags = latest.findall('img[@alt="Image"]')  # Get all image tags
                image_links = [
                    element.xpath("@src")[0] for element in image_tags
                ]  # Get the links to the image
                image_links = [
                    url.replace("//", "https://") for url in image_links
                ...
                async with aiohttp.ClientSession() as session:
                    for link in image_links:
                        async with session.get(link) as response:
                            ...
                            image_list.append(
                                io.BytesIO(connection)
                            )  # Convert the images into bytes
                multiple_images = True

        if multiple_images:  # If there are multiple images
            pil_images = list(
                map(Image.open, image_list)
            )  # Open all byte images as PIL images
            ...
            canvas = Image.new(
                "RGBA", (canvas_width, canvas_height)
        ...
        bold_span_tags = re.findall(
            r'(<span style="font-weight: bold">([\w\W]+?)</span>)', text_decoded
        )  # Find all <span> tags used to bold text
        if bold_span_tags:  # If there are bolded text
            for tag in bold_span_tags:
                text_decoded = text_decoded.replace(
                    tag[0], f"%@^{tag[1]}%@^"
                )  # Change the <span> tag to a temporary name

        emoji_list = re.findall(
            r"\s*<img[\w\W]+?>", text_decoded
        )  # Check if there are emojis in the news article
        ...
        text_decoded = text_decoded.replace(
            "//", "https://"
        )  # Replace all relative links to prefix with HTTPS
        links = set(re.findall(r'href="(.*?)"', text_decoded))  # Get all href links
        for link in links:
            text_decoded = text_decoded.replace(
                link, f"https://www.chickensmoothie.com{link}"
            )  # Prepend Chicken Smoothie base URL

        content = html2text.html2text(
            text_decoded
        )  # Convert remaining HTML into Markdown
        ...
        links = re.findall(
            r"\(http[s]*[\w\W]+?\)", content
        )  # Get all links in the Markdown
        for link in links:
        ...
        # 11) Send embed
        embed = discord.Embed(
            title=post_date, description=content, colour=0x4BA139
        )  # Create embed
        if multiple_images:  # If there are multiple images
            ...
            await ctx.send(
                embed=embed, file=discord.File(fp=output_buffer, filename="news.png")
            )  # Upload the file to the channel where message came from
