...
class Pet(commands.Cog):
    ...
    async def pet(self, ctx, link: str = ""):  # Pet command
        ...
        if pet is None:
            embed = discord.Embed(
                title="Pet", description=Strings.pet_unsuccessful, colour=0xFF5252
            )  # Create embed
        else:
            embed = discord.Embed(
                title=pet.owner_name + "'s Pet", colour=0x4BA139
            )  # Create embed
            ...
            if pet.pps:
                embed.add_field(
                    name="PPS",
                    value='[This pet has "PPS". What\'s that?](http://www.chickensmoothie.com/help/pets#pps)',
                    inline=False,
                )
            if pet.store_pet:
                embed.add_field(
                    name="Store",
                    value="[This pet was sold in the store](https://www.chickensmoothie.com/store/)",
                    inline=False,
                )
            ...
            if pet.given_name is not None:
                embed.add_field(
                    name=f"Given to {pet.owner_name} by", value=pet.given_by()
                )
