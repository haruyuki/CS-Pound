...
class PoundPets(commands.Cog):
    ...
    async def pound_pets(self, ctx):
        embed = discord.Embed(
            title="Pound Pets",
            description="Pound Pets is temporarily disabled due to technical issues. Please refer to the 'support' command to join the Development server for more information.",
            colour=0xFF5252,
        )  # Create embed
        ...
        #             {"generated": True}
        #         )  # Try to get document of generated image
        ...
        #         await ctx.send(
        #             file=discord.File(fp=output_buffer, filename="poundpets.png")
        #         )
        #     else:
        #         await ctx.send(
        #             "No image has been generated yet! Type `,ppets get` to start generating"
        #         )
    ...
    #         if (
    #             pound_data[0] == "Lost and Found"
    #             or pound_data[0] == "Pound & Lost and Found"
    #         ):
    #             await ctx.send("The next opening is not the Pound!")

    #         elif (
    #             pound_data[0] == "Pound" and "Pound is currently open!" in pound_data[1]
    #         ):
    #             await ctx.send(
...
#     for i in pil_images:
#         if (
#             current_width + i.width >= max_width
#         ):  # If pasting an image will cause it to go off canvas
#             current_width = 0
#             y_offset += current_max_height + 31 + 15
#             current_max_height = 0

#         if (
#             i.height > current_max_height
#         ):  # If pet is taller than current top height in row
#             current_max_height = i.height

#         if i.width < 106:
#             paste_width = 106
#             canvas.paste(
#                 i, (math.floor(current_width + ((106 - i.width) / 2)), y_offset)
#             )

#             text_centre_offset_x = math.floor(
#                 (
#                     106
#                     - draw.textsize(rare_plus_pets[pil_images.index(i)][2], font=font)[
#                         0
#                     ]
#                 )
#                 / 2
#             )
#             draw.text(
#                 (current_width + text_centre_offset_x, i.height + y_offset),
#                 rare_plus_pets[pil_images.index(i)][2],
#                 fill=(0, 0, 0),
#                 font=font,
#             )
...
#                 canvas.paste(
#                     very_rare, (current_width, i.height + y_offset + 15), very_rare
#                 )
#             elif pet_rarity == "OMG so rare!":
#                 canvas.paste(
#                     omg_so_rare, (current_width, i.height + y_offset + 15), omg_so_rare
#                 )
#         else:
#             paste_width = i.width
...
#             draw.text(
#                 (
#                     current_width + pasting_width + text_centre_offset_x,
#                     i.height + y_offset,
#                 ),
#                 rare_plus_pets[pil_images.index(i)][2],
#                 fill=(0, 0, 0),
...
#                 canvas.paste(
#                     rare,
#                     (current_width + pasting_width, i.height + y_offset + 15),
#                     rare,
#                 )
#             elif pet_rarity == "Very rare":
#                 canvas.paste(
