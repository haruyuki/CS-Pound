...
class RemindMe(commands.Cog):
    ...
    async def remindme(self, ctx, *amount: str):  # Remind Me command
        ...
        if total == 0:  # If no time specified
            embed = discord.Embed(
                title="Remind Me", description=Strings.invalid_time, colour=0xFF5252
            )  # Create embed
            await ctx.send(embed=embed)  # Send embed
        elif total > 86400:  # If time is longer than 24 hours
            embed = discord.Embed(
                title="Remind Me",
                description=Strings.remindme_too_long,
                colour=0xFF5252,
            )  # Create embed
            await ctx.send(embed=embed)  # Send embed
            ...
            embed = discord.Embed(
                title="Remind Me", description=before_message, colour=0x4BA139
            )  # Create embed
            await ctx.send(embed=embed)  # Send embed
            after_message = (
                f"Reminder for {ctx.message.author.mention}!"  # Reminder for USER!
            )
