...
class TestClass:
    def test_get_standard_data(self):
        data = loop.run_until_complete(get_user('Arita', 'osu'))
        assert data is not None
        assert data.user_id == 7109858
        assert data.username == 'Arita'
    ...
    def test_get_taiko_data(self):
        data = loop.run_until_complete(get_user('Arita', 'taiko'))
        assert data is not None
        assert data.user_id == 7109858
        assert data.username == 'Arita'

    def test_get_catch_data(self):
        data = loop.run_until_complete(get_user('Arita', 'catch'))
        assert data is not None
        assert data.user_id == 7109858
        assert data.username == 'Arita'

    def test_get_mania_data(self):
        data = loop.run_until_complete(get_user('Arita', 'mania'))
        assert data is not None
        assert data.user_id == 7109858
        assert data.username == 'Arita'
